SB <- function(mu.link = "logit", sigma.link = "log") {
  ...
  list(
    ...
    mu.linkfun    = mstats$linkfun,    mu.linkinv    = mstats$linkinv,    mu.dr    = mstats$mu.eta,
    sigma.linkfun = dstats$linkfun,    sigma.linkinv = dstats$linkinv,    sigma.dr = dstats$mu.eta,
    ...
    G.dev.incr = function(y, mu, sigma, bd, ...){
    ...
    rqres = expression({
      rqres(pfun="pSB", type="Discrete", ymin=0, y=y, mu=mu, sigma=sigma, bd=bd)
    }),

    ## Iniciales y dominios
    mu.initial    = expression({ mu    <- pmin(pmax((y + 0.5)/(bd + 1), 1e-6), 1-1e-6) }),
